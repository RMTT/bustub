...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    if (this->children_.count(key_char)) return &this->children_[key_char];
    return nullptr;
  }
  ...
  void RemoveChildNode(char key_char) {
    if (this->HasChild(key_char)) this->children_.erase(key_char);
  }
...
class Trie {
 ...
 public:
   ...
   * character.
   */
  Trie() : root_(std::make_unique<TrieNode>('\0')), latch_(ReaderWriterLatch()) {}
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *p = &this->root_;
    for (const auto &x : key.substr(0, key.size() - 1)) {
      if (!p->get()->HasChild(x)) p->get()->InsertChildNode(x, std::make_unique<TrieNode>(x));

      p = p->get()->GetChildNode(x);
    }

    char last_c = key[key.size() - 1];
    std::unique_ptr<TrieNodeWithValue<T>> r;
    if (p->get()->HasChild(last_c)) {
      if (p->get()->GetChildNode(last_c)->get()->IsEndNode()) {
        this->latch_.WUnlock();
        return false;
      }

      r = std::make_unique<TrieNodeWithValue<T>>(std::move(**p->get()->GetChildNode(last_c)), value);
      p->get()->RemoveChildNode(last_c);
    } else {
      r = std::make_unique<TrieNodeWithValue<T>>(last_c, value);
    }

    p->get()->InsertChildNode(last_c, std::move(r));
  ...
  bool Remove(const std::string &key) {
    this->latch_.WLock();
    bool result = this->RemoveHelper(key, &this->root_);
    this->latch_.WUnlock();
    return result;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    TrieNodeWithValue<T> *r = dynamic_cast<TrieNodeWithValue<T> *>(p->get());
 ...
 private:
  bool RemoveHelper(const std::string_view &key, std::unique_ptr<TrieNode> *p) {
    if (key.empty()) return false;

    char current_c = key[0];
    std::unique_ptr<TrieNode> *cur = p->get()->GetChildNode(current_c);
    if (cur == nullptr) return false;

    bool result = false;
    if (key.size() > 1) {
      char next_c = key[1];
      result = RemoveHelper(key.substr(1), cur);

      std::unique_ptr<TrieNode> *next = cur->get()->GetChildNode(next_c);
      if (next && !next->get()->HasChildren() && !next->get()->IsEndNode()) cur->get()->RemoveChildNode(next_c);
    } else {
      if (!cur->get()->IsEndNode()) return false;
      // TODO: convert TrieNodeWithValue to TrieNode
